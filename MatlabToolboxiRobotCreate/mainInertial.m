...
while isempty(goalXY_inertial)
    while 1
        ...
        grid on;
        axis equal;
        title('Current 2D fakeLIDAR observation');
        xlabel('x / m');
        ylabel('z / m');
        axis([-5 5 0 8]);
        ...
        % If there's a can, and it's between -0.3 and 0.15, figure out
        % where it is in the inertial frame of reference
        exist p3;
        if ans
            delete(p3);
        end
        if size(objectPos,2) > 0 && objectPos(2,1)<0.15 && objectPos(2,1)>-0.3
            ...
            hold on;
            p3 = plot(inerObjectPos(1,:),inerObjectPos(2,:),'bo');  % this is the can detected
            if sum(isnan(inerObjectPos(:,1))) == 0
                goalXY_inertial = [inerObjectPos(1,1);inerObjectPos(2,1)];
            end
        ...
        if ~isempty(goalXY_inertial)
            hold on;
            exist p7;
            if ans
                delete(p7);
            end
            p7 = plot(goalXY_inertial(1),goalXY_inertial(2),'rx', 'MarkerSize',15);
        end
...
subplot(1,2,2);
myOccupancyShow=pathPlan(map,goalXY_inertial);
...
while 1
    ...
    % Convert inertial frame goal to body frame
    iner2bodyM = [cos(currentPose(3)), sin(currentPose(3));
        -sin(currentPose(3)), cos(currentPose(3))];
...
%         currentPose(3) + angleChange];
    ...
    % Plot SLAM model and robot position
    subplot(1,3,3);
    title('SLAM Model and Vehicle Position, and Can Position');
    show(slamAlg);
    %     SSetFwdVelRadiusRoomba(serPort, roombaSpeed, 0);
    currentPose = optimizedPoses(size(optimizedPoses,1),:);
    ...
    % Plot the goal position if it exists
    if ~isempty(goalXY_inertial)
        exist p7;
        hold on;
        if ans
