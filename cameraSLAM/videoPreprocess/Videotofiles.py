...
def CreateVideoimages(videopath, imagefolder,orb):
    ...
    while(newframeavailable): #as long as there are new frames availabe
        ...
        # start saving frames into images
        imagename = imagefolder + "/frame{0}.jpg".format(frame_id)
        reducedFrame = cv2.resize(frame, (0,0), fx=0.3,fy=0.3)
        #reducedFrame = cv2.flip(reducedFrame, -1) # flip the image

        written = cv2.imwrite(imagename , reducedFrame)
        if not written:
